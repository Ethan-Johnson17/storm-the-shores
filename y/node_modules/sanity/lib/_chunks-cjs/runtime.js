...
function getAliases({ monorepo, sanityPkgPath }) {
  ...
  if (sanityPkgPath) {
    ...
    return browserCompatibleSanityPackageSpecifiers.reduce(
      (acc, next) => {
        const dest = resolve__default.default.exports(pkg, next, { browser: !0, conditions })?.[0];
...
function getDocumentHtml(studioRootPath, props, importMap) {
  const Document = getDocumentComponent(studioRootPath), css = props?.css?.map((url) => {
    try {
  ...
  return {
    ...
    async generateBundle(_options, outputBundle) {
      const bundle = outputBundle, entryFile = Object.values(bundle).find(
        (file) => file.type === "chunk" && file.name === "sanity" && file.facadeModuleId?.endsWith(entryChunkId)
      );
      if (!entryFile)
...
async function getViteConfig(options) {
  const {
    ...
    importMap
  } = options, monorepo = await loadSanityMonorepo(cwd), basePath = normalizeBasePath(rawBasePath), sanityPkgPath = (await readPkgUp__default.default({ cwd: __dirname }))?.path;
  if (!sanityPkgPath)
