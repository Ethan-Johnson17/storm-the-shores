...
function RouterProvider(props) {
  const { onNavigate, router: routerProp, state } = props, resolveIntentLink = useCallback(
    ...
    (nextState) => {
      const currentStateParams = state._searchParams || [], nextStateParams = nextState._searchParams || [], nextParams = STICKY_PARAMS.reduce((acc, param) => replaceStickyParam(
        acc,
        param,
    ...
    [routerProp, state]
  ), handleNavigateStickyParam = useCallback(
    (param, value, options = {}) => {
  ...
  ), [routerState, stickyParams] = useMemo(() => {
    if (!state._searchParams)
      return [state, null];
    const { _searchParams, ...rest } = state, [sticky, restParams] = partition(_searchParams, ([key]) => STICKY_PARAMS.includes(key));
    return sticky.length === 0 ? [state, null] : [{ ...rest, _searchParams: restParams }, sticky];
  }, [state]), router = useMemo(
    () => ({
      navigate,
      navigateIntent,
